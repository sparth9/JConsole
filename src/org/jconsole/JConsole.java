 ...
 * JConsole.java
 *
 * Created on September 17, 2001, 12:05 PM
 */
...
public class JConsole implements Runnable {
    ...
    private static final Class[] CMD_CLASS_LIST = new Class[] {
        org.jconsole.HelpCommand.class, // must be the first one
        org.jconsole.ExitCommand.class,
        org.jconsole.QuitCommand.class,
        org.jconsole.HistoryCommand.class,
        org.jconsole.ShowCommand.class,
        org.jconsole.PWDCommand.class,
        org.jconsole.SetCommand.class,
        org.jconsole.RMDIRCommand.class,
        org.jconsole.LSCommand.class ,
        org.jconsole.CDCommand.class ,
        org.jconsole.CPCommand.class ,
        org.jconsole.CLSCommand.class ,
        org.jconsole.PTrace.class
    };
    ...
    private List m_histryRWList = new ArrayList();
    private List m_histryROList = null;
    ...
    public static void main(String [] args) {
     ...
     public void run() {
        try {
            ...
            try {
                String line = new BufferedReader(new InputStreamReader(Console.in)).readLine();
            } catch(Exception e2) {
            }
        }
    }
    ...
    private void addCommand(Map m, List l, ConsoleCommand cmd) throws Exception {
	...
	private void loadInternalCommands(Map m, List l) throws Exception {
		//create commands
	...
	private void loadExternalCommands(Map m, List l ) throws Exception {
	...
	public final File getFile(String aKey) {
		File result = null;
		LSCommand ls = (LSCommand)getCommand("ls");
		try {
	...
	private String[] processAlias(String line) {
		...
		String[] aliasVal = (String[]) m_aliases.get(arr[0]);
		if (aliasVal == null) {
			return arr;
		}
		...
		public void execute(String[] args) throws CommandFailedException {
			...
			if (args.length == 0) { // display aliases
				...
				while (it.hasNext()) {
					...
					String[] val = (String[])m_aliases.get(key);
					dispAlias(key, val);
				}
				return;
			}
			if (args.length == 1 ) { // display given alias
				String[] val = (String[])m_aliases.get(args[0]);
				if (val == null) {
	...
	class HomeCommand extends ConsoleCommand {
		...
		private Map m_alternateHomes = new HashMap();
		...
		public void execute(String[] args) throws CommandFailedException {
			...
			if (args.length == 1) {
				String path = (String) m_alternateHomes.get(args[0]);
				if (path == null) {
